...
Pagina* insereaza(Pagina *pag, int x, char nume[30], Nod *nod)
{
	...
	else
	{
		...
		if (r != q) //daca pagina in care s-a inserat s-a scindat <=> arborele crescut cu un nivel
		{
			...
			pag = insereaza(pag, r->e[1].cheie, r->e[1].nume, &r->e[1]);
...
void afisare(Pagina *arbore, int nivel, int data)
{
	...
	if (arbore->p0)
		afisare(arbore->p0, nivel + 1, data);

	for (i = 1; i <= arbore->m; i++) {
		if (arbore->e[i].cheie > data) {
			are_chei = 1;
			break;
		}
	}
	...
	if (are_chei) {
		...
		for (i = 1; i <= arbore->m; i++) {
			if (arbore->e[i].cheie > data)
				printf("%d %s\t", arbore->e[i].cheie, arbore->e[i].nume);
		}
		printf("\n");
	}
	...
	for (i = 1; i <= arbore->m; i++) {
		if (arbore->e[i].p)
			afisare(arbore->e[i].p, nivel + 1, data);
	}
}
